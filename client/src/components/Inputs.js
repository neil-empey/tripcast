  ...
  handleSubmit(event) {
    ...
    axios
      ...
      .then(response => {
        ...
        this.setState({ url: ((`https://www.mapquestapi.com/staticmap/v5/map?start=${this.state.origin}|flag-start&end=${this.state.dest}|flag-end&size=@2x&key=`).toString() + (process.env.consumer_key).toString())});
      });
